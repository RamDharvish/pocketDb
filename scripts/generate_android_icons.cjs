...
const SVG_FOREGROUND = `<svg width="432" height="432" viewBox="0 0 432 432" fill="none" xmlns="http://www.w3.org/2000/svg">
  <defs>
  ...
  <!-- Scale and translate PocketDB Logo to fit 432x432 canvas within central 260px safe zone -->
...
const SVG_FULL = `<svg width="512" height="512" viewBox="0 0 512 512" fill="none" xmlns="http://www.w3.org/2000/svg">
  <defs>
  ...
  <path d="M 0 128 L 512 128 M 0 256 L 512 256 M 0 384 L 512 384 M 128 0 L 128 512 M 256 0 L 256 512 M 384 0 L 384 512" stroke="#38BDF8" stroke-opacity="0.06" stroke-width="1.5" />
...
const SVG_ROUND = `<svg width="512" height="512" viewBox="0 0 512 512" fill="none" xmlns="http://www.w3.org/2000/svg">
  <defs>
    <linearGradient id="bgGrad" x1="0" y1="0" x2="512" y2="512" gradientUnits="userSpaceOnUse">
      <stop offset="0%" stop-color="#0F172A" />
      <stop offset="50%" stop-color="#0B0F19" />
...
async function main() {
  ...
  const fullSvgBuf = Buffer.from(SVG_FULL);
  const roundSvgBuf = Buffer.from(SVG_ROUND);
  const fgSvgBuf = Buffer.from(SVG_FOREGROUND);
  ...
  for (const [folder, sizes] of Object.entries(densities)) {
    ...
    console.log(`Generated icons in ${folder}: ${sizes.icon}px (icon/round), ${sizes.foreground}px (foreground)`);
  }

  // Also write 512x512 app icon
  const assetsDir = path.join(__dirname, '../assets');
  if (!fs.existsSync(assetsDir)) fs.mkdirSync(assetsDir, { recursive: true });
  await sharp(fullSvgBuf).resize(512, 512).png().toFile(path.join(assetsDir, 'icon.png'));

  // Also write to public/icon.png & public/favicon.svg
  const publicDir = path.join(__dirname, '../public');
  if (!fs.existsSync(publicDir)) fs.mkdirSync(publicDir, { recursive: true });
  await sharp(fullSvgBuf).resize(512, 512).png().toFile(path.join(publicDir, 'icon.png'));
  fs.writeFileSync(path.join(publicDir, 'favicon.svg'), SVG_FULL);
